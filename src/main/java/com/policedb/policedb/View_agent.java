...
public class View_agent implements Initializable{

    private static final String SELECT_VIEW = "SELECT * from agents";
    ...
    @FXML
    public void viewAgentsHiredByYou(ActionEvent event) throws IOException
    {
        URL url = new File("src/main/resources/fxml/agents_view_by_hire.fxml").toURI().toURL();
        root = FXMLLoader.load(url);
        stage = (Stage)((Node)event.getSource()).getScene().getWindow();
        scene = new Scene(root);
        stage.setScene(scene);
